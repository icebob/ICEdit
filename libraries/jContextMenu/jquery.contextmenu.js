	...
	$.contextMenu = {
		...
		show: function(t,e) {
			...
			if (cmenu.beforeShow()!==false) {
				...
				$c.css( {top:pos.y+"px", left:pos.x+"px", position:"absolute",zIndex:9999} );
				$c[cmenu.showTransition](cmenu.showSpeed,((cmenu.showCallback)?function(){cmenu.showCallback(cmenu);}:null));
				if (!cmenu.showSpeed && cmenu.showCallback) {/* icebob */
					cmenu.showCallback(cmenu);
				}
				cmenu.shown=true;
		...
		hide: function() {
			...
			if (cmenu.shown) {
				...
				if (cmenu.menu) { 
					cmenu.menu[cmenu.hideTransition](cmenu.hideSpeed,((cmenu.hideCallback)?function(){cmenu.hideCallback(cmenu);}:null)); 
					if (!cmenu.showSpeed && cmenu.hideCallback) /* icebob */
						cmenu.hideCallback(cmenu);
				}
